...
用户进一步允许用类似 `...` 的方式压缩代码。原规则已允许省略非核心装配，本轮在全部 9 份 Prompt 中明确采用 `// ...：省略日志格式化` 等语言注释，说明省略范围，并将含省略的内容标为“机制节选”。核心仍用真实代码；关键条件、必需实参、动作、返回和必要清理不能变成省略号。判断标准是读者能追通当前机制的输入、触发、数据传递、状态变化与结果，不要求补成完整工程。

只读复核确认现有 Ob/Cm 样例在已声明范围内保留这条逻辑链；注释式省略与禁止伪代码不冲突。配置同步、工程正文保留及结构检查重新执行。本轮仅澄清规则，未重新生成示例、重跑 JavaScript 或编译 C；既有试跑与运行记录仍属于上一轮。现有校验器允许注释形式的省略标记，继续拒绝独立一行的裸占位符，不能靠它判断省略内容是否真正无关。

## v7 最新规则的 EDR 再生成

用户随后要求“测试生成一次，EDR笔记”。本轮独立生成从实际安装的 Espanso YAML 读取 `/shuli`，以 `inputs/edr-draft.md` 和 `inputs/codefirst-request.md` 为输入，不读取旧产物照抄改写，也不修改 Prompt。读取结果为 5,761 字符，UTF-8 SHA256 为 `d9dfaabaf52803224daceb6d1aa423ee6a7d832e7160364265b1765ed9e2c4bb`，与知识源文件一致。

比较基线为提交 `1c313ba` 中的 `outputs/edr-driver-knowledge.md`：26,445 字符、4,401 个中文字符、6 块 C 代码共 448 行。只进行一次新生成；后续技术复核修正在同一稿件中完成，不算另一次 Prompt 试跑。

最终稿保留 11 个一级知识域，包含 7 块 C 代码、3 个 Mermaid、9 处注释式省略。省略范围为事件输出、序列化、队列实现、设备与服务装配等；WFP 会话与 AppId 的准备改为明确的入口前提，匹配、注册绑定、事务回滚和释放仍真实展开。Ob/Cm 的对象与参数来源、操作分派、条件、动作及退出路径经独立复核，没有用省略号代替核心处理。

复核补齐 Cm 的 Cookie 发布门控，限定 Pre 拒绝后缺失的是本回调的 Post；将“非零成功状态下 Object 未定义”与“操作失败”分开，避免错误增加失败计数。Cm 回调约束修正为 `PASSIVE_LEVEL`，与注册、名称查询 API 的约束分开。WFP 的通知签名、终止型 Filter 与实际 BLOCK 动作一致；补充必要头文件/链接条件，并显式清空释放后的 AppId，方便会话关闭失败时安全重试。

| 指标 | 上一版 | 本次复核后 |
| --- | ---: | ---: |
| 全文字符 | 26,445 | 33,983 |
| 中文字符 | 4,401 | 6,134 |
| 代码围栏外中文字符 | 3,329 | 4,844 |
| C 代码块 / 行数 | 6 / 448 | 7 / 465 |
| 注释式省略标记 | 0 | 9 |

**本次结果：结构与核心逻辑检查通过，整体精简目标未完全达成。** 新增线程/镜像示例占 42 行；其余同类 C 片段合计由 448 行降至 423 行，说明部分机制确实压缩了装配。但解释、对比与来源展开使全文增长约 28.5%，围栏外中文增长约 45.5%。不能因出现了省略号或静态脚本通过，就认定最终笔记更简洁；本轮如实保留该结果，没有再生成一版掩盖它。

`verify_prompts.py` 和 `git diff --check` 通过，Prompt 与本地 Espanso 配置未修改，CLI 仍加载相同内容。`verification.json` 的 `passed` 只表示自动结构检查通过，不表示精简程度通过。驱动代码未编译、链接、加载或运行；Mermaid 未在 Obsidian 中渲染验证。AI 游戏样例仅随脚本做静态回归，本轮没有重新生成或执行它的代码。
